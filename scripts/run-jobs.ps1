...
try {
    ...
    $rules = "Hard rules, in order: " +
             "(1) Read CONTEXT.md 'Job-application pipeline' glossary, docs/adr/0009-job-application-automation.md, and JOBS.md first. The active slice is named in the 'Next slice' block as '<slice-id> -- #N'. Run gh issue view N for its detail. " +
             "(2) Branch off the latest origin/master (git fetch then git checkout -b jobs/<slice-id>-short-name origin/master). " +
             ...
             "(6) Enforce ADR-0009: the application-submit tool is irreversible=True; the ONLY unattended auto-submit is the already-landed S7 gate (zero-guessed + opt-in + no-new-eligibility, scoped to job_apply_submit alone). Do NOT widen that gate; everything else is review-before-submit. " +
             ...
             "(9) Rewrite the JOBS.md 'Next slice' block: tick the landed entry in the queue, set the next unticked entry as the Active slice (its slice id + '#N' and its 'Model:' line), set 'Status:' exactly as the block's own instructions say (ready while unticked queue entries remain; done when the queue is complete), and add the merged PR under 'Landed PRs'. " +
