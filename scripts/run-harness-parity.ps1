# run-delegation.ps1 -- autonomous slice loop for the deepseek-harness parity program.
#
# Repeats: read HARNESS-PARITY.md 'Next slice' block -> launch a fresh headless
...
# HARNESS-PARITY.md + the issue are the only memory between them. Logs land in
# .claude\tmp\harness-parity-loop\.
#
# Stop conditions:
#   - STOP file created by scripts/stop-delegation.ps1 (graceful, between steps)
#   - HARNESS-PARITY.md Status: done      (all slices landed)
#   - HARNESS-PARITY.md Status: blocked   (a HITL slice opened a PR for review)
...
try {
    ...
    $driver = Join-Path $repoRoot "HARNESS-PARITY.md"
    if (-not (Test-Path $driver)) { throw "HARNESS-PARITY.md not found at $driver" }

    $stateDir = Join-Path $repoRoot ".claude\tmp\harness-parity-loop"
    ...
    if ($null -eq $claudeCmd) {
        Notify "Harness parity loop" "claude.cmd not found on PATH. Install Claude Code CLI (npm) first."
        exit 1
    }
    ...
    $rules = "Hard rules, in order: " +
             "(1) Read HARNESS-PARITY.md and the ADR named in the active slice Queue line (e.g. docs/adr/0021-conversation-compaction.md) first. The active slice is named in the 'Next slice' block as 'Sx -- #N' with a 'Model:' line; its Type (AFK or HITL) is in the Queue line for that slice. Run gh issue view N for its full acceptance criteria. " +
             "(2) Branch off the latest origin/master (git fetch then git checkout -b hp/sN-short-name origin/master). " +
             ...
             "  * Type: HITL -> DO NOT merge. Leave the PR open. Set HARNESS-PARITY.md 'Status: blocked' with a one-line reason naming the PR number ('Sx PR #<pr> awaiting human review'), commit that to master, and stop. " +
             "(8) git checkout master and git pull origin master so master is current. " +
             "(9) Rewrite the HARNESS-PARITY.md 'Next slice' block: tick the landed entry in the Queue, set the next unticked+unblocked entry as Active (its 'Sx -- #N' and 'Model:' lines), set 'Status:' (ready while AFK slices remain; blocked if you just opened a HITL PR; done after the last slice lands), and append the merged PR (AFK only) under 'Landed PRs'. " +
             "Commit the HARNESS-PARITY.md change directly to master and push it. HARNESS-PARITY.md is the ONLY thing you may commit straight to master -- all slice code goes through the PR. " +
    ...
    Log ("=== Harness parity loop started (max {0} slices, {1} attempts each, auto-resume={2}) ===" -f $MaxSlices, $MaxAttempts, $AutoResume)
    ...
    for ($slice = 1; $slice -le $MaxSlices -and -not $stopAll; $slice++) {
        ...
        if ($status -eq "done")    { Notify "Harness parity loop" "HARNESS-PARITY.md says Status: done. All slices landed."; break }
        if ($status -eq "blocked") { Notify "Harness parity loop" "HARNESS-PARITY.md says Status: blocked. A HITL PR is awaiting your review (or a session needs input) - read HARNESS-PARITY.md."; break }

        $model = Get-DriverField "Model" "sonnet"
        if ($allowedModels -notcontains $model) {
            Log ("Invalid Model: '{0}' in HARNESS-PARITY.md, falling back to sonnet" -f $model)
            $model = "sonnet"
        }
        ...
        for ($attempt = 1; $attempt -le $MaxAttempts; $attempt++) {
            ...
            if ($attempt -eq 1) {
                $prompt = "Read HARNESS-PARITY.md and implement the active slice (Sx -- #N) exactly as specced in its issue, end to end with hermetic tests. " + $rules
            } else {
                $prompt = ("This is debug attempt {0} of {1} for the active slice in HARNESS-PARITY.md. " -f $attempt, $MaxAttempts) +
            ...
            if ($output -match $limitPattern) {
                if ($output -match "Not logged in|Failed to authenticate") {
                    Notify "Harness parity loop" "The claude CLI is not logged in (or its token expired). Open a terminal, run claude, type /login, then restart the loop."
                    $stopAll = $true
                    break
                }
                ...
                if (-not $AutoResume) {
                    ...
                    Notify "Harness parity loop" $msg
                    $stopAll = $true
                    break
                }
                ...
                if ($limitWaits -gt $MaxLimitWaits) {
                    Notify "Harness parity loop" ("Usage limit hit {0} times in a row - stopping to avoid an endless wait. Check your plan, then restart." -f $MaxLimitWaits)
                    $stopAll = $true
                    break
                }
                ...
                NotifyTimed "Harness parity loop" ("Usage limit reached{0}. Sleeping until ~{1}, then auto-resuming. Closing this window cancels." -f `
        ...
        if (-not $succeeded) {
            Notify "Harness parity loop" ("Slice failed after {0} attempts. Check the logs in .claude\tmp\harness-parity-loop and HARNESS-PARITY.md, then restart the loop." -f $MaxAttempts)
            break
        }
    }

    Log "=== Harness parity loop ended ==="
} catch {
