# run-documents.ps1 -- autonomous slice loop for Documents.
#
# Repeats: read DOCUMENTS.md 'Next slice' block -> launch a fresh headless Claude
...
#
# Stop conditions:
#   - STOP file created by scripts/stop-documents.ps1 (graceful, between steps)
#   - DOCUMENTS.md Status: done    (all slices landed)
#   - DOCUMENTS.md Status: blocked (a session decided it needs a human)
...
#
# Each attempt is a brand-new session; DOCUMENTS.md is the only memory between
# them. Logs land in .claude\tmp\documents-loop\.
#
...
try {
    ...
    $driver = Join-Path $repoRoot "DOCUMENTS.md"
    if (-not (Test-Path $driver)) { throw "DOCUMENTS.md not found at $driver" }

    $stateDir = Join-Path $repoRoot ".claude\tmp\documents-loop"
    ...
    if ($null -eq $claudeCmd) {
        Notify "Documents loop" "claude.cmd not found on PATH. Install Claude Code CLI (npm) first."
        exit 1
    }
    ...
    $rules = "Hard rules, in order: " +
             ...
             "(2) Branch off the latest origin/master (git fetch then git checkout -b documents/sN-short-name origin/master). " +
             "(3) Implement ONLY that one slice exactly as the issue specifies. One PR per issue. Code areas: plugins/documents.py, plugins/job_search.py, cerebral/main.py (seams + IPC), tray/windows/main.html, tray/lib/*.js, scripts/*.ps1. " +
             "(4) SAFETY (highest priority): obey the DOCUMENTS.md SAFETY block -- never install software (no winget), never run a real soffice/LibreOffice/UNO in tests (stub the seams), never touch the user's Desktop files, no live job board or ATS. Behaviour only checkable live -> APPEND an item to docs/documents-live-verify.md, do NOT perform it. " +
             ...
             "(8) Rewrite the DOCUMENTS.md 'Next slice' block: tick the landed entry in the queue, set the next unticked entry as Active (including its Model from the queue line -- sonnet unless the entry says otherwise), " +
             "set 'Status:' (ready while slices remain; done after S7 lands), and add the merged PR under 'Landed PRs'. " +
             "Commit the DOCUMENTS.md change directly to master and push it. DOCUMENTS.md is the ONLY thing you may commit straight to master. " +
    ...
    Log ("=== Documents loop started (max {0} slices, {1} attempts each, auto-resume={2}) ===" -f $MaxSlices, $MaxAttempts, $AutoResume)
    ...
    for ($slice = 1; $slice -le $MaxSlices -and -not $stopAll; $slice++) {
        ...
        if ($status -eq "done")    { Notify "Documents loop" "DOCUMENTS.md says Status: done. All slices landed."; break }
        if ($status -eq "blocked") { Notify "Documents loop" "DOCUMENTS.md says Status: blocked. A session needs your input - read DOCUMENTS.md."; break }

        $model = Get-DriverField "Model" "sonnet"
        if ($allowedModels -notcontains $model) {
            Log ("Invalid Model: '{0}' in DOCUMENTS.md, falling back to sonnet" -f $model)
            $model = "sonnet"
        }
        ...
        for ($attempt = 1; $attempt -le $MaxAttempts; $attempt++) {
            ...
            if ($attempt -eq 1) {
                $prompt = "Read DOCUMENTS.md and complete the active slice exactly as specced in its issue. " + $rules
            } else {
                $prompt = ("This is debug attempt {0} of {1} for the active slice in DOCUMENTS.md. " -f $attempt, $MaxAttempts) +
            ...
            if ($output -match $limitPattern) {
                ...
                if ($output -match "Not logged in|Failed to authenticate") {
                    Notify "Documents loop" "The claude CLI is not logged in (or its token expired). Open a terminal, run claude, type /login, then restart the loop."
                    $stopAll = $true
                    break
                }
                ...
                if ($limitWaits -gt $MaxLimitWaits) {
                    Notify "Documents loop" ("Usage limit hit {0} times in a row - stopping to avoid an endless wait. Check your plan, then restart." -f $MaxLimitWaits)
                    $stopAll = $true
                    break
                }
                ...
                NotifyTimed "Documents loop" ("Usage limit reached{0}. Sleeping until ~{1}, then auto-resuming. Closing this window cancels." -f `
        ...
        if (-not $succeeded) {
            Notify "Documents loop" ("Slice failed after {0} attempts. Check the logs in .claude\tmp\documents-loop and DOCUMENTS.md, then restart the loop." -f $MaxAttempts)
            break
        }
    }
