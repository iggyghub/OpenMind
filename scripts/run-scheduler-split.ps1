# run-scheduler-split.ps1 -- autonomous slice loop for Scheduler Split.
#
# Repeats: read SCHEDULER-SPLIT.md 'Next slice' block -> launch a fresh headless Claude
...
#
# Stop conditions:
#   - STOP file created by scripts/stop-scheduler-split.ps1 (graceful, between steps)
#   - SCHEDULER-SPLIT.md Status: done    (all slices landed)
#   - SCHEDULER-SPLIT.md Status: blocked (a session decided it needs a human)
...
#
# Each attempt is a brand-new session; SCHEDULER-SPLIT.md is the only memory between
# them. Logs land in .claude\tmp\scheduler-split-loop\.
#
...
try {
    ...
    $driver = Join-Path $repoRoot "SCHEDULER-SPLIT.md"
    if (-not (Test-Path $driver)) { throw "SCHEDULER-SPLIT.md not found at $driver" }

    $stateDir = Join-Path $repoRoot ".claude\tmp\scheduler-split-loop"
    ...
    if ($null -eq $claudeCmd) {
        Notify "Scheduler Split loop" "claude.cmd not found on PATH. Install Claude Code CLI (npm) first."
        exit 1
    }
    ...
    $rules = "Hard rules, in order: " +
             "(1) Read SCHEDULER-SPLIT.md first. The active slice is named in the 'Next slice' block as 'Sx -- #N'. Run gh issue view N for its detail. " +
             "(2) Branch off the latest origin/master (git fetch then git checkout -b split/sN-short-name origin/master). " +
             ...
             "(5) Open the PR with 'Part of #1208' in the body (only the final slice's issue says 'Closes #1208' -- use that issue's own wording, don't add it yourself). Merge YOUR OWN PR: gh pr merge <n> --squash --delete-branch. " +
             ...
             "(7) Rewrite the SCHEDULER-SPLIT.md 'Next slice' block: tick the landed entry in the queue, set the next unticked entry as Active, " +
             "set 'Status:' (ready while slices remain; done after last slice lands), and add the merged PR under 'Landed PRs'. " +
             "Commit the SCHEDULER-SPLIT.md change directly to master and push it. SCHEDULER-SPLIT.md is the ONLY thing you may commit straight to master. " +
    ...
    Log ("=== Scheduler Split loop started (max {0} slices, {1} attempts each, auto-resume={2}) ===" -f $MaxSlices, $MaxAttempts, $AutoResume)
    ...
    for ($slice = 1; $slice -le $MaxSlices -and -not $stopAll; $slice++) {
        ...
        if ($status -eq "done")    { Notify "Scheduler Split loop" "SCHEDULER-SPLIT.md says Status: done. All slices landed."; break }
        if ($status -eq "blocked") { Notify "Scheduler Split loop" "SCHEDULER-SPLIT.md says Status: blocked. A session needs your input - read SCHEDULER-SPLIT.md."; break }

        $model = Get-DriverField "Model" "sonnet"
        if ($allowedModels -notcontains $model) {
            Log ("Invalid Model: '{0}' in SCHEDULER-SPLIT.md, falling back to sonnet" -f $model)
            $model = "sonnet"
        }
        ...
        for ($attempt = 1; $attempt -le $MaxAttempts; $attempt++) {
            ...
            if ($attempt -eq 1) {
                $prompt = "Read SCHEDULER-SPLIT.md and complete the active slice exactly as specced in its issue. " + $rules
            } else {
                $prompt = ("This is debug attempt {0} of {1} for the active slice in SCHEDULER-SPLIT.md. " -f $attempt, $MaxAttempts) +
            ...
            if ($output -match $limitPattern) {
                ...
                if ($output -match "Not logged in|Failed to authenticate") {
                    Notify "Scheduler Split loop" "The claude CLI is not logged in (or its token expired). Open a terminal, run claude, type /login, then restart the loop."
                    $stopAll = $true
                    break
                }
                ...
                if (-not $AutoResume) {
                    ...
                    Notify "Scheduler Split loop" $msg
                    $stopAll = $true
                    break
                }
                ...
                if ($limitWaits -gt $MaxLimitWaits) {
                    Notify "Scheduler Split loop" ("Usage limit hit {0} times in a row - stopping to avoid an endless wait. Check your plan, then restart." -f $MaxLimitWaits)
                    $stopAll = $true
                    break
                }
                ...
                NotifyTimed "Scheduler Split loop" ("Usage limit reached{0}. Sleeping until ~{1}, then auto-resuming. Closing this window cancels." -f `
        ...
        if (-not $succeeded) {
            Notify "Scheduler Split loop" ("Slice failed after {0} attempts. Check the logs in .claude\tmp\scheduler-split-loop and SCHEDULER-SPLIT.md, then restart the loop." -f $MaxAttempts)
            break
        }
    }

    Log "=== Scheduler Split loop ended ==="
} catch {
