# run-video.ps1 -- autonomous slice loop for the Video-watching campaign.
#
# Repeats: read VIDEO.md 'Next slice' block -> launch a fresh headless Claude
...
#
# Stop conditions:
#   - STOP file created by scripts/stop-video.ps1 (graceful, between steps)
#   - VIDEO.md Status: done    (all slices landed)
#   - VIDEO.md Status: blocked (a session decided it needs a human)
...
#
# Each attempt is a brand-new session; VIDEO.md is the only memory between them.
# Logs land in .claude\tmp\video-loop\.
#
...
try {
    ...
    $driver = Join-Path $repoRoot "VIDEO.md"
    if (-not (Test-Path $driver)) { throw "VIDEO.md not found at $driver" }

    $stateDir = Join-Path $repoRoot ".claude\tmp\video-loop"
    ...
    if ($null -eq $claudeCmd) {
        Notify "Video loop" "claude.cmd not found on PATH. Install Claude Code CLI (npm) first."
        exit 1
    }
    ...
    $rules = "Hard rules, in order: " +
             "(1) Read VIDEO.md (including SAFETY), the CONTEXT.md entries 'Watching', 'Escalation', 'Verdict', 'Commit', and docs/adr/0017-video-watching.md first. The active slice is named in the 'Next slice' block as 'Sx -- #N'. Run gh issue view N for its detail. " +
             "(2) Branch off the latest origin/master (git fetch then git checkout -b video/sN-short-name origin/master). " +
             "(3) Implement ONLY that one slice exactly as the issue specifies. One PR per issue. Code areas: cerebral/video/*, plugins/video.py, cerebral/main.py (seams + IPC), tray/windows/main.html, tray/lib/*.js, scripts/*.ps1. " +
             "(4) SAFETY (highest priority): obey the VIDEO.md SAFETY block -- never hit the network or a real service (no real yt-dlp/TikTok fetch, no real web search, no real Anthropic/vision call), never install software, wire everything through injectable seams and stub them in tests. Behaviour only checkable live -> APPEND an item to docs/video-live-verify.md, do NOT perform it. " +
             "(5) Run the relevant tests (python -m pytest cerebral/tests -q, plus npx jest in tray/ when the panel changed) and proceed ONLY if ALL pass. " +
             ...
             "(8) Rewrite the VIDEO.md 'Next slice' block: tick the landed entry in the queue, set the next unticked entry as Active (including its Model from the queue line -- sonnet unless the entry says otherwise), " +
             "set 'Status:' (ready while slices remain; done after S7 lands), and add the merged PR under 'Landed PRs'. " +
             "Commit the VIDEO.md change directly to master and push it. VIDEO.md is the ONLY thing you may commit straight to master. " +
    ...
    Log ("=== Video loop started (max {0} slices, {1} attempts each, auto-resume={2}) ===" -f $MaxSlices, $MaxAttempts, $AutoResume)
    ...
    for ($slice = 1; $slice -le $MaxSlices -and -not $stopAll; $slice++) {
        ...
        if ($status -eq "done")    { Notify "Video loop" "VIDEO.md says Status: done. All slices landed."; break }
        if ($status -eq "blocked") { Notify "Video loop" "VIDEO.md says Status: blocked. A session needs your input - read VIDEO.md."; break }

        $model = Get-DriverField "Model" "sonnet"
        if ($allowedModels -notcontains $model) {
            Log ("Invalid Model: '{0}' in VIDEO.md, falling back to sonnet" -f $model)
            $model = "sonnet"
        }
        ...
        for ($attempt = 1; $attempt -le $MaxAttempts; $attempt++) {
            ...
            if ($attempt -eq 1) {
                $prompt = "Read VIDEO.md and complete the active slice exactly as specced in its issue. " + $rules
            } else {
                $prompt = ("This is debug attempt {0} of {1} for the active slice in VIDEO.md. " -f $attempt, $MaxAttempts) +
            ...
            if ($output -match $limitPattern) {
                if ($output -match "Not logged in|Failed to authenticate") {
                    Notify "Video loop" "The claude CLI is not logged in (or its token expired). Open a terminal, run claude, type /login, then restart the loop."
                    $stopAll = $true
                    break
                }
                ...
                if ($limitWaits -gt $MaxLimitWaits) {
                    Notify "Video loop" ("Usage limit hit {0} times in a row - stopping to avoid an endless wait. Check your plan, then restart." -f $MaxLimitWaits)
                    $stopAll = $true
                    break
                }
                ...
                NotifyTimed "Video loop" ("Usage limit reached{0}. Sleeping until ~{1}, then auto-resuming. Closing this window cancels." -f `
        ...
        if (-not $succeeded) {
            Notify "Video loop" ("Slice failed after {0} attempts. Check the logs in .claude\tmp\video-loop and VIDEO.md, then restart the loop." -f $MaxAttempts)
            break
        }
    }

    Log "=== Video loop ended ==="
} catch {
