# run-replay.ps1 -- autonomous slice loop for Historical Replay.
#
# Repeats: read REPLAY.md 'Next slice' block -> launch a fresh headless Claude
...
#
# Stop conditions:
#   - STOP file created by scripts/stop-replay.ps1 (graceful, between steps)
#   - REPLAY.md Status: done    (all slices landed)
#   - REPLAY.md Status: blocked (a session decided it needs a human)
...
#
# Each attempt is a brand-new session; REPLAY.md is the only memory between
# them. Logs land in .claude\tmp\replay-loop\.
#
...
try {
    ...
    $driver = Join-Path $repoRoot "REPLAY.md"
    if (-not (Test-Path $driver)) { throw "REPLAY.md not found at $driver" }

    $stateDir = Join-Path $repoRoot ".claude\tmp\replay-loop"
    ...
    if ($null -eq $claudeCmd) {
        Notify "Historical Replay loop" "claude.cmd not found on PATH. Install Claude Code CLI (npm) first."
        exit 1
    }
    ...
    $rules = "Hard rules, in order: " +
             "(1) Read REPLAY.md first. The active slice is named in the 'Next slice' block as 'Sx -- #N'. Run gh issue view N for its detail. " +
             "(2) Branch off the latest origin/master (git fetch then git checkout -b replay/sN-short-name origin/master). " +
             ...
             "(7) Rewrite the REPLAY.md 'Next slice' block: tick the landed entry in the queue, set the next unticked entry as Active, " +
             "set 'Status:' (ready while slices remain; done after last slice lands), and add the merged PR under 'Landed PRs'. " +
             "Commit the REPLAY.md change directly to master and push it. REPLAY.md is the ONLY thing you may commit straight to master. " +
    ...
    Log ("=== Historical Replay loop started (max {0} slices, {1} attempts each, auto-resume={2}) ===" -f $MaxSlices, $MaxAttempts, $AutoResume)
    ...
    for ($slice = 1; $slice -le $MaxSlices -and -not $stopAll; $slice++) {
        ...
        if ($status -eq "done")    { Notify "Historical Replay loop" "REPLAY.md says Status: done. All slices landed."; break }
        if ($status -eq "blocked") { Notify "Historical Replay loop" "REPLAY.md says Status: blocked. A session needs your input - read REPLAY.md."; break }

        $model = Get-DriverField "Model" "sonnet"
        if ($allowedModels -notcontains $model) {
            Log ("Invalid Model: '{0}' in REPLAY.md, falling back to sonnet" -f $model)
            $model = "sonnet"
        }
        ...
        for ($attempt = 1; $attempt -le $MaxAttempts; $attempt++) {
            ...
            if ($attempt -eq 1) {
                $prompt = "Read REPLAY.md and complete the active slice exactly as specced in its issue. " + $rules
            } else {
                $prompt = ("This is debug attempt {0} of {1} for the active slice in REPLAY.md. " -f $attempt, $MaxAttempts) +
            ...
            if ($output -match $limitPattern) {
                ...
                if ($output -match "Not logged in|Failed to authenticate") {
                    Notify "Historical Replay loop" "The claude CLI is not logged in (or its token expired). Open a terminal, run claude, type /login, then restart the loop."
                    $stopAll = $true
                    break
                }
                ...
                if (-not $AutoResume) {
                    ...
                    Notify "Historical Replay loop" $msg
                    $stopAll = $true
                    break
                }
                ...
                if ($limitWaits -gt $MaxLimitWaits) {
                    Notify "Historical Replay loop" ("Usage limit hit {0} times in a row - stopping to avoid an endless wait. Check your plan, then restart." -f $MaxLimitWaits)
                    $stopAll = $true
                    break
                }
                ...
                NotifyTimed "Historical Replay loop" ("Usage limit reached{0}. Sleeping until ~{1}, then auto-resuming. Closing this window cancels." -f `
        ...
        if (-not $succeeded) {
            Notify "Historical Replay loop" ("Slice failed after {0} attempts. Check the logs in .claude\tmp\replay-loop and REPLAY.md, then restart the loop." -f $MaxAttempts)
            break
        }
    }

    Log "=== Historical Replay loop ended ==="
} catch {
