...
**Shell mode** — a deliberately degraded boot of Felix that routes every task to a user-chosen fallback model instead of the normal priority chain. Entered only from the **boot menu**; it exists because Felix's function is gated on model capability and a single unreachable model otherwise takes the whole system down. The model is picked in the AI selector as the `shell` task pin, and — like every task pin — routes even while disabled in the priority list. **Ephemeral by construction**: it governs exactly one boot and never writes to the priority store, so a diagnostic mode can never leak into normal operation. The selector warns when the chosen shell model shares a **failure domain** with the primary (same host), since a fallback on the failing host is not a fallback. _Avoid_: calling it "safe mode" — nothing is disabled for safety; only the model changes. _Avoid_: treating it as a fallback tier in the priority chain — the chain is automatic, shell mode is chosen by a human at boot.

**Failure domain** — the set of models that go down together. Two models on the same host (both Budd entries on `bonsai.ai-dabs.com`) share one; a local ollama model and a remote endpoint do not. Used to judge whether a fallback is real. _Avoid_: equating it with local-vs-remote — two *different* remote providers are separate failure domains.

**Admission cap** — the maximum number of concurrent outbound model calls Felix allows against one **Failure domain**, enforced by an `asyncio.Semaphore`. Default 1, per R5's "the scarce resource is always singular." A **System setting**, user-editable. Priority among callers waiting for a full domain's slot is queue-order, not preemption: `task_type == "chat"` (the live conversational turn) is served before any waiting background `task_type`, but a call already holding the slot is never interrupted. Felix may raise a **Proposal** to change the value (from observed stalls or sustained headroom) but never silently adjusts it. See ADR-0036. _Avoid_: treating this as a scheduler or work queue — both were explicitly rejected in favor of one bounded semaphore.

**Boot menu** — the interstitial the tray shows while waiting for Cerebral to accept connections on `:7766` (a 30-45s cold start that is otherwise silent). Shown on **every** start as passive boot feedback; it takes focus and accepts the Space+Enter chord for **shell mode** only on a cold, user-initiated launch, never on an automated restart (self-dev, boot-check rollback, Restart Felix) — an automated restart must never steal keystrokes from whatever the user is typing in. Owned by the Electron/launcher layer, not Cerebral, for the same reason the boot self-check is: a broken brain cannot rescue itself. The chord is window-scoped, so it costs nothing from the global-hotkey budget (ADR-0028 R7). _Avoid_: calling it a splash screen — it delays nothing and is a mode selector, not branding.

**Failure contract** — how Felix responds to each kind of failure, and the rule that they are not the same kind. A **tool error** is information about the world (a locked file, a network blip, a rate limit): the chain continues, the failed step reaches the planner as `ERROR: …`, and it retries, tries another tool, or answers with what it has — bounded by a cap of two failures of the same call. A **gate denial** is a decision about authority and is always terminal: it is never fed back, because routing around a refusal is what a planner naturally does after a tool error, and the two are indistinguishable from its side. The retry that is diligence after a tool error is circumvention after a denial. A **model outage** is neither — it is the processor being gone, answered by **shell mode**. _Avoid_: calling a gate denial an error; it is a decision, and treating it as recoverable is the whole risk.

**Verification contract** — the uniform interface every capability-acquisition mechanism (Plugin, self_dev diff, Skill, Recipe, growth-loop tool) implements to satisfy R6's "verified running, or it did not ship": `verify() -> VerifyResult`, where `VerifyResult = {passed, evidence, score}`. `passed` and `evidence` are mandatory for every mechanism (`evidence` is the receipt that something actually ran, not just a claim); `score` is optional and mechanism-defined — most leave it `None`, and only a mechanism with its own notion of quality (trading's **Confidence weight**) populates it. Discovery mirrors `REQUIRED_CAPABILITIES`: the orchestrator introspects the method, so a new mechanism type needs to implement it, not get added to a hand-maintained table. See ADR-0034. _Avoid_: treating `score` as something every mechanism must eventually fill in — it is deliberately per-mechanism, not a universal quality judge.

**Supervisor** — the Electron tray, in its second job: the one long-lived process that starts, watches, restarts, and health-checks the other one. It is not a new component; the role was always the tray's, spread unnamed across `tray/main.js` (the 3s reconnect loop, `respawnCerebral()`, `_relaunch()`, the boot self-check, the 5-minute master-update poll, the single-instance lock) and `scripts/launch-felix.ps1` (prerequisite checks, the 120s wait for `:7766`, and the #521 reconciliation of a half-running system). The tray owns it because it is the only process that is long-lived, knows whether Felix is idle, can put a notification in front of the user, and is not the thing likely to be broken — the same "a broken brain cannot rescue itself" reasoning that put the **boot self-check + rollback** here. The launcher is its *cold-start* half, not a second supervisor: it runs once per launch and exits. Nothing supervises the tray, deliberately — that rung is the human double-click (ADR-0028 R1's last rung), and the tray fails far less often than Cerebral, which is the process loading Kokoro, ChromaDB and 50+ plugins. _Avoid_: reading CONTEXT.md's "registered as an OS service" line as supervision — an OS service buys **autostart at boot**, not restart-on-crash, and cannot notify or read `felixState`. _Avoid_: calling `launch-felix.ps1` the supervisor; it cannot observe anything after it exits.

**Code load** — a restart that carries *new code* into the live tree, as distinct from a plain **restart** that reboots the same commit. Only a code load arms the **boot self-check + rollback**; a restart does not, because there is nothing to roll back to that differs from what is already running. Three things are code loads: the **self-dev loop** pulling a merged PR, the **supervisor**'s master-update poll fast-forwarding to upstream, and a human `git pull`. A restart is everything else — the tray and File menu items, and the `restart_felix` tool. The distinction has to travel on the wire, because the tray is what arms the rollback and Cerebral is what knows the intent; where the two are indistinguishable, the destructive reading must be the opt-in one. Local commits are **not** a code load: code authored on this machine is the developer's working state, not an update to adopt. _Avoid_: "restart" as a blanket word for both — it is the exact ambiguity that let a chat-level "restart yourself" arm a `git reset --hard`.

**Process log** — the plaintext record of what the Cerebral/tray *processes* did: `cerebral.log`, `cerebral.err.log`, `launcher.log`, `tray.*.log` at the repo root. Machine-global, unencrypted, one timestamped file per launch. Distinct from the **conversation store**, which records what *Felix* did — structured, Fernet-encrypted, profile-scoped, and surfaced in the Log nav tab. The split is deliberate and the reason is directional: the conversation store may depend on Felix working, the process log must survive Felix failing, so a crash log never lives behind a Fernet key and a running brain. The two are joined by the chain's `run_id`, not merged. _Avoid_: "the logs" as one thing — the question "did Felix do X" and the question "why did the process die" have different answers in different places.
...
**Absorbed app** — an external GUI program Felix launches so the user (or Felix, headlessly) can do a creative/document task on the program's own turf — LibreOffice today (ADR-0011); GIMP/Darktable, Blender, Figma named next. Governed by the **absorbed-app contract**: opened as its own program via the invocation ladder (never rebuilt in Felix's chrome), its output lands in the **Document library** by default, Felix and the user take turns rather than co-edit, and Felix never reskins it. Distinct from a **plugin** with no GUI (Home Assistant, Reddit, Twitter/X — API integrations, not absorptions) and from `tools/ui-editor/` (edits Felix's *own* live surfaces, not an external program — governed by the renderer's no-framework rule instead). See ADR-0037. _Avoid_: treating every OSS integration as an absorption — only ones with their own native GUI Felix launches qualify.
...
**Blast-radius gate** — superseded 2026-08-21/22, kept for history. Originally: auto-merge on green tests only in *safe zones*, any **guardrail** diff (`cerebral/security/`, the ADR-0010 sandbox, the credential/keyring store, `cerebral/main.py` core, self-dev's own code) always escalating to human review regardless of test colour. The 2026-08-21 "full auto-merge" amendment (`plugins/self_dev.py`, `is_guardrail_diff`) removed the guardrail block entirely; a guardrail hit today is **informational only** — logged to the Activity Log as "needs human review" — and does not stop the merge. A 2026-08-22 tightening added back the one gate that survives: **test status**. A PR whose tests didn't pass stays open (`merge_decision: "tests_failed"`); one whose tests passed auto-merges regardless of what it touched, guardrails included. The safety net for a bad self-merge is entirely the **Boot self-check + rollback** now, not a pre-merge review. _Avoid_: assuming a guardrail-path diff gets human review before merging — verify against `is_guardrail_diff` in `plugins/self_dev.py` directly, since this has changed twice already.

**Boot self-check + rollback** — the reversal guarantee for the **self-dev loop**, owned by the launcher/Electron layer (where restart lives), *not* by Cerebral's Python (the thing that might be broken). Before a restart-to-load, Cerebral's current `master` SHA is pinned as `last_known_good` and the structured state (`openmind.db` + `felix-settings.json`) is snapshotted (5 rolling). On relaunch the new code runs a self-check (imports OK + ADR-0005 gate present + IPC up on 7766); **pass** promotes the new SHA to `last_known_good`, **fail** resets to `last_known_good`, restores the matching state snapshot, relaunches the old code, and notifies. Git history *is* the code backup — no folder copies. Armed only by a **code load**, never by a plain restart. The reset stashes an uncommitted working tree before moving HEAD, so a rollback is never destructive: recovery of a human's in-flight edits is a `git stash pop`, and the notification names the stash. _Avoid_: putting the health-check inside Cerebral (a broken brain can't rescue itself); snapshotting `chroma/`, `browser/`, or documents (rebuildable, disposable, or not corruptible by a code change); treating `git reset --hard` as safe because "git history is the backup" — uncommitted work has no history.
...
**Main window** — the primary Felix UI surface. A chat/interaction canvas where the user converses with Felix by voice (the fast lane — wake + speak) or by typing (the slow-but-silent lane). The transcript renders both lanes interleaved. Layout: a collapsible left **sidebar nav**, open-ended in count and grown by campaigns as needed — **Conversation**, **Harness**, **Library**, **Trading**, **Log**, **Settings** as of 2026-08-24 (#473 collapsed an original 16 routes down to four; **Trading** was promoted back out of Library in S19 and **Log** added in S26, both as their own top-level sections rather than sub-tabs, since each earned Felix-wide or cross-cutting status). A new top-level section is reserved for a capability that doesn't belong nested under an existing one; anything scoped to an existing section (e.g. a new trading view) is added as a sub-tab inside it, not a new section. Everything else is a sub-view reached inside one of these. The right side is the **workspace**. The Queue earns a count badge in the chat header (the only time-sensitive surface) so the user sees pending items without leaving the conversation. Distinct from the **Visualiser** (ambient overlay) and from the **tray** (always-on launcher + quick-actions). Lifecycle: the Main window does **not** autostart with Cerebral — the user opens it on demand from the tray. Closing the window **hides** it; it does not quit Felix. Quit is reachable only from the tray. _Avoid_: calling it "the chat window" — it is the chat *and* the control surface.
...
**Panel spec** — the JSON a plugin returns to describe its **panel**: a declarative tree of widgets drawn from the **panel vocabulary**. A plugin never ships HTML or JavaScript into the Main window renderer — the renderer owns the drawing, the plugin owns the data (ADR-0012). This is what keeps an LLM-generated plugin from executing code beside the Credentials UI. Extends the schema-driven form rendering already proven in `schemaToFormHtml`. _Avoid_: "plugin UI code" — plugins contribute *data*, never code, to the renderer.

**Panel vocabulary** — the fixed set of widgets `panel-spec.js` knows how to draw: `list`, `detail`, `text`, `action`, `group`, `cluster`, `toggle`, `table`, **registry**. Felix's widget toolkit. Adding a widget is a deliberate, reviewed extension of the renderer, never something a plugin can do from outside. _Avoid_: treating the vocabulary as closed forever (it has grown from `{list, detail}`) or as open on demand (a plugin cannot introduce one).

**Registry widget** — the shared row shape for browsing an installed capability mechanism: name, status, a **Verification contract** result badge, and its actions as `ps-action` buttons. Plugins panel, the Skills sub-tab, and Recipes tab render through it instead of each hand-authoring its own list markup — the ADR-0031 "unify the rules, not the rendering" move applied to browsing UI, not just buttons. Does not move nav placement: each mechanism keeps its existing sidebar location (Plugins/Skills under Harness, Recipes under Library); this is a shared component, not a consolidated section. See ADR-0035. _Avoid_: treating this as a reason to add a new top-level "Capabilities" nav section — that was considered and rejected.

**Interface guidelines** — the rules every Felix-drawn surface obeys, whether it was drawn by the **panel vocabulary** or hand-authored. Three parts: shared visual tokens across all Felix windows (not just the Main window); a type and space scale alongside the existing shape/elevation/color tokens (ADR-0027); and **the button invariant — a button *is* a declared tool call**, carrying the tool and its args so the click routes through the ADR-0005 gate like any other invocation, never a bare `onclick` handler. Guidelines rather than a toolkit rewrite: the Main window's hand-authored surfaces migrate onto the contract incrementally instead of being rebuilt on the vocabulary. Scope is Felix's own pixels — native tray menus and OS notifications deliberately follow the host, and an **absorbed** external app (LibreOffice, ADR-0011) can only be made consistent *behaviourally*: how it is invoked and where its output lands. _Avoid_: calling it a design system and meaning only colors — the button invariant is the behavioural half and the more load-bearing one.
...
**Trend basket play** — a second hand-written (not book-sourced, not LLM-generated), fixed-rule Strategy, same "grill it out, validate once against real data" origin as the IPO play, but a different shape: a 10-symbol equal-weighted (10% each) basket, opened when market breadth (the share of the Candidate pool trading above its own 50-day moving average, read from the prior day's close) exceeds 60%. Picks are the pool's top 10 by momentum × 20-day realized volatility (not momentum alone — pure momentum ranking looked strong on a narrow hand-picked test universe but collapsed to near-coin-flip on a full 58-symbol universe; the volatility tilt partially, not fully, recovers a real edge — see ADR-0038's "Full-universe correction"). Each position exits independently on a flat 12% trailing stop from its own peak (no tightening ratchet — see ADR-0038 for why the IPO play's tightening shape was tried and rejected here), hard-capped at 20 trading days. Unlike the IPO play, every candidate this strategy can select already has real price history (the 50-day/20-day lookbacks require it), so it carries no Gauntlet exception — each selected symbol is `_run_gauntlet`-validated normally, the same as any other idea source. **What's actually validated:** absolute participation (the basket has positive median return and a 56-61% win rate specifically during gate-on/breadth-trending-up conditions, holding across every universe and selection variant tested) is solid; beating a passive basket of the same universe is a real but modest, not statistically proven, edge (0/58 symbols individually significant after Benjamini-Hochberg correction). See ADR-0038. _Avoid_: assuming this reuses the IPO play's exit mechanic (the trailing-stop *concept* transfers, the specific 3%/1%-tighten-at-20% parameters do not — they destroy this strategy's edge at both daily and 5-minute granularity); and _avoid_ trusting backtest numbers from a small hand-picked universe over the full-universe ones — this design's own history this session is a cautionary example of exactly that gap.

**Candidate pool** — the shared, refreshed universe of tickers that pattern-general Discovery, Expansion, and Same-symbol composite selection all draw from (ADR-0026 decision 5: one pool, no per-feature duplication). As of the 2026-09-08 grill (TICKER-DISCOVERY.md), built fresh from Alpaca's real movers (top gainers + losers — a loser is a bounce/reversal candidate, not just noise), most-actives, and a random sample of the broader tradable universe, narrowed by the existing liquidity/ATR fitness filter — replacing a hand-maintained static ticker list that needed a human to re-edit it as which companies matter shifts over time. _Avoid_: treating this as a market-cap or "blue-chip" universe — it is deliberately volatility/liquidity-driven (real swings), not size-driven; a stable mega-cap that barely moves is exactly what this pool is not optimized to surface.
