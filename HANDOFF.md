...
The active multi-slice epic is **#184 Main window** / **#191 Slice 2** (per-panel migrations). Each slice retires a legacy `tray/windows/<panel>.html` popup and lifts it into the `data-route="<panel>"` pane in `tray/windows/main.html`. Five landed so far (Queue, Insights, Memory, Credentials, Permissions) — all pure popup-retirements are now done. After landing a slice, **update this block** so the next session starts cold without re-deriving context.

1. **Pick the next panel.** Two slices remain, both different shapes from the popup retirements: **Profile-setup** is a wizard with `MediaRecorder` + `AudioContext` + canvas waveform — the audio capture has to be re-hosted inside a non-modal pane while staying mic-permission-friendly. **Plugins** and **Settings** are new-feature builds with no popup to retire (their placeholder copy is what gets fleshed out). The natural order: Profile-setup next (still a migration; finishes the popup-retirement set even though it's a wizard not a list), then Plugins, then Settings.
2. **Read the reference PRs in order:** [#195](https://github.com/iggyghub/OpenMind/pull/195) (Queue, simple list), [#197](https://github.com/iggyghub/OpenMind/pull/197) (Insights, inline edit), [#199](https://github.com/iggyghub/OpenMind/pull/199) (Memory, long-form), [#201](https://github.com/iggyghub/OpenMind/pull/201) (Credentials, two-card pane with write-only secrets), [#203](https://github.com/iggyghub/OpenMind/pull/203) (Permissions, dual-mode lib + inner tabs + modal). The diffs are the implementation template.
3. **Clone tracker issue #202 verbatim** with panel-specific renaming (route name, CSS prefix, WS event names). Pick a distinct accent colour: Queue purple, Insights blue `#5cb8fc`, Memory green `#4bd49a`, Credentials amber `#e0b755`, Permissions brand purple are taken — Profile/Plugins/Settings can each pick from teal, magenta, or a deeper amber. Title format: `Migrate <Panel> panel into main.html sidebar (Slice 2.N of #184)`. File via `gh issue create --repo iggyghub/OpenMind --body-file`.
4. **Branch off the most recent merged migration branch** (currently `drop-202-permissions-migration`; rebase up the chain as parent PRs merge): `git checkout -b drop-NNN-<panel>-migration`.
5. **Implement** in `tray/windows/main.html` (lift CSS + markup + WS wiring + `list_<panel>` on WS open + per-pane re-pull on activation) and retire in `tray/main.js` (drop globals, IPC channels, window opener; repoint tray menu to `openMainWindow('#<panel>')`). **For libs in `tray/lib/`:** reuse the UMD-ish dual-mode wrapper from `permissions-store.js` — Node tests keep working AND the renderer can load via `<script src>`.
6. **Delete** `tray/windows/<panel>.html` via `git rm`.
7. **Verify:** run the cumulative pytest suite (add this slice's tests to the trailing list) — must all pass. Run `npm test` in `tray/` if you touched any `tray/lib/` file. Grep for stale references in the live tree (exclude `.claude/**`, `HANDOFF.md`, `docs/adr/0007-*.md` — those are expected historical mentions).
...
- ⏳ 2.7 Profile-setup wizard next — `MediaRecorder` + `AudioContext` re-hosted in a pane; branch base `drop-202-permissions-migration`
- ⏳ 2.8 Plugins (new build) / 2.9 Settings (new build) — not migrations
