...
The active multi-slice epic is **#184 Main window** / **#191 Slice 2** (per-panel migrations). Each slice retires a legacy `tray/windows/<panel>.html` popup and lifts it into the `data-route="<panel>"` pane in `tray/windows/main.html`. Six landed so far (Queue, Insights, Memory, Credentials, Permissions, Profile-setup) — **all popup retirements are now done**. The two remaining slices are new-feature builds. After landing a slice, **update this block** so the next session starts cold without re-deriving context.

1. **Pick the next panel.** Two new-feature builds remain: **Plugins** (every registered plugin with status, declared capabilities, tool count, per-plugin settings — including the Discord allowlist editor) and **Settings** (system-wide preferences currently scattered across the tray submenu: notifications, reminder interval, camera, visualiser toggle, models picker, per-task model assignments). Both involve mostly-existing Cerebral state served via existing WS broadcasts (`plugins_list`, `models_list`, `env_context_update`, etc) so the Python side stays untouched; this is renderer-only work. Plugins is the more complex of the two (Discord allowlist editor has its own grain) — pick Settings first as the easier consolidation slice, or Plugins first if the Discord allowlist work is a higher priority.
2. **Read the reference PRs in order:** [#195](https://github.com/iggyghub/OpenMind/pull/195) (Queue, simple list), [#197](https://github.com/iggyghub/OpenMind/pull/197) (Insights, inline edit), [#199](https://github.com/iggyghub/OpenMind/pull/199) (Memory, long-form), [#201](https://github.com/iggyghub/OpenMind/pull/201) (Credentials, two-card pane with write-only secrets), [#203](https://github.com/iggyghub/OpenMind/pull/203) (Permissions, dual-mode lib + inner tabs + modal), [#205](https://github.com/iggyghub/OpenMind/pull/205) (Profiles, list + inline wizard with `MediaRecorder`). The diffs are the implementation template — the Profile PR especially shows how to host a recorder/canvas surface inside a pane.
3. **Clone tracker issue #204 verbatim** with panel-specific renaming (route name, CSS prefix, WS event names). Pick a distinct accent colour: Queue purple, Insights blue `#5cb8fc`, Memory green `#4bd49a`, Credentials amber `#e0b755`, Permissions brand purple, Profiles magenta `#c44b9c` are taken — Plugins/Settings can pick teal (`#4bd4d4`), coral, deeper amber, or a soft cyan. Title format: `<Panel> pane in main.html sidebar (Slice 2.N of #184)` — these slices are *builds*, not *migrations*, so adjust the title verb.
4. **Branch off the most recent merged migration branch** (currently `drop-204-profile-setup-migration`; rebase up the chain as parent PRs merge): `git checkout -b drop-NNN-<panel>-build`.
5. **Implement** in `tray/windows/main.html` (lift the relevant Cerebral state into the pane via the existing WS, render lists/forms, post mutations through `sendEvent`). For Settings specifically: removing items from the tray submenu (Notifications, Reminder interval, Camera, Model picker) means also moving the wiring out of `buildMenu()` in `tray/main.js`. For Plugins: the Discord allowlist editor is its own card — read `tray/main.js` and the Discord plugin code for the current allowlist shape before designing.
6. **No popup file to delete** for these two — they were placeholders all along.
7. **Verify:** run the cumulative pytest suite — must all pass. Run `npm --prefix tray test` if you touched any `tray/lib/` file (avoid `cd tray &&` — see `.learnings/LEARNINGS.md` 2026-06-09 entry). Grep for stale references in the live tree.
8. **Commit + push + open PR** with `--base <previous-migration-branch>`, mirror the [Memory commit](https://github.com/iggyghub/OpenMind/pull/199/commits) message structure, include `Closes #NNN`.
...
- ⏳ 2.8 Settings (new build, tray submenu consolidation) — branch base `drop-204-profile-setup-migration`
- ⏳ 2.9 Plugins (new build, with Discord allowlist editor)
