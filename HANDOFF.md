...
The active multi-slice epic is **#184 Main window** / **#191 Slice 2** (per-panel migrations). Each slice retires a legacy `tray/windows/<panel>.html` popup and lifts it into the `data-route="<panel>"` pane in `tray/windows/main.html`. Four landed so far (Queue, Insights, Memory, Credentials). After landing a slice, **update this block** so the next session starts cold without re-deriving context.

1. **Pick the next panel.** Three popup-retirement slices remain: **Permissions** (heaviest — has `permissions-store` lib + three WS events: `permissions_state` / `tools_list` / `plugins_list`), **Profile-setup** (wizard with `MediaRecorder` + `AudioContext` — different shape from list panes). **Plugins** and **Settings** are new-feature builds, not migrations (no popup to retire). After this section get Permissions out of the way before tackling the wizard — it's the last pure migration.
2. **Read the reference PRs in order:** [#195](https://github.com/iggyghub/OpenMind/pull/195) (Queue, simple list), [#197](https://github.com/iggyghub/OpenMind/pull/197) (Insights, inline edit), [#199](https://github.com/iggyghub/OpenMind/pull/199) (Memory, long-form), [#201](https://github.com/iggyghub/OpenMind/pull/201) (Credentials, two-card pane with write-only secrets + focus-survives-rerender pattern). The diffs are the implementation template.
3. **Clone tracker issue #200 verbatim** with panel-specific renaming (route name, CSS prefix, WS event names). Pick a distinct accent colour: Queue purple, Insights blue `#5cb8fc`, Memory green `#4bd49a`, Credentials amber `#e0b755` are taken. Title format: `Migrate <Panel> panel into main.html sidebar (Slice 2.N of #184)`. File via `gh issue create --repo iggyghub/OpenMind --body-file`.
4. **Branch off the most recent merged migration branch** (currently `drop-200-credentials-migration`; rebase up the chain as parent PRs merge): `git checkout -b drop-NNN-<panel>-migration`.
5. **Implement** in `tray/windows/main.html` (lift CSS + markup + WS wiring + `list_<panel>` on WS open + per-pane re-pull on activation) and retire in `tray/main.js` (drop globals, IPC channels, window opener; repoint tray menu to `openMainWindow('#<panel>')`). **For Permissions:** decide first whether `permissions-store.js` lib stays (the renderer can keep using it; it's just a client-side resolver) or gets inlined.
...
- ⏳ 2.6 Permissions next — branch base `drop-200-credentials-migration`
- ⏳ 2.7 Profile-setup wizard — different shape (needs audio recording surface inside the pane)
